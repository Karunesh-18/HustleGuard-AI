...
# ─── Existing zone / rider / claim / payout schemas ───────────────────────────
...
class RiderPolicyCreate(BaseModel):
    ...
    waive_waiting_period: bool = Field(
        default=False,
        description="When True, sets eligible_from=now so rider can claim immediately. "
                    "Use after confirmed payment — the waiting period prevents fraud, "
                    "but paying the premium is sufficient proof of good faith.",
    )
...
# FraudEvaluationRequest is defined in schemas/fraud.py which itself imports
# from this module; using a late import breaks the circular dependency.
